...
class Stock:
    def __init__(self, name: str, symbol: str, current_price: float, available_shares: int):
        self.id = uuid.uuid4()
        self.name = name
        self.symbol = symbol
    ...
    def buy(self, shares_to_buy: int):
        ...
        logger.info(f"Market impact: {market_impact:.3f} ({market_impact*100:.1f}% of market)")
        ...
        logger.info(f"Scarcity factor: {scarcity_factor:.3f} (normal supply: {normal_supply})")
        ...
        logger.info(f"Log factor: {log_factor:.3f} (shares: {shares_to_buy}, sensitivity: {market_sensitivity})")
        ...
        logger.info(f"Total price change: {total_change:.3f} ({total_change*100:.1f}%)")
